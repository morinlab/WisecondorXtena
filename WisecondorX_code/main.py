...
def tool_test(args):
    ...
    if args.offset_neut_peak is not None:
        results_r = np.append(results_r, results_r_2) + args.offset_neut_peak
        results_z = np.append(results_z, results_z_2) - m_z + args.offset_neut_peak
        results_w = np.append(results_w * np.nanmean(results_w_2), 
                              results_w_2 * np.nanmean(results_w))
        results_w = results_w / np.nanmean(results_w) + args.offset_neut_peak
    else:
...
def main():
    ...
    parser_test.add_argument('--offset_neut_peak',
                             type=float,
                             default=None,
                             help='adds the mean log2ratio of the neutral peak from the mixture model to results_r, results_z and results_w to shift resulting log2ratios and plots, can be positive or negative, positive will cause an upwards shift and negative will cause a downwards shift')
